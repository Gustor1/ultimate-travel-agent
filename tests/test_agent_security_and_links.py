...
from ultimate_travel_agent.validator import validate_all_agents, validate_agent_file
...
def test_internal_agents_least_privilege():
    ...
    for ia in internal_agents:
        ...
        if ia_path.exists():
            ...
            assert "web_search" not in content.lower() or ("tools:" in content.lower() and "web_search" not in content.lower().split("tools:")[1].split("---")[0]), f"Internal agent {ia} has web_search"
            assert "browser" not in content.lower() or ("tools:" in content.lower() and "browser" not in content.lower().split("tools:")[1].split("---")[0]), f"Internal agent {ia} has browser"
...
def test_packages_mirror_synchronized():
    ...
    match, mismatch, errors = filecmp.cmpfiles(source_dir, mirror_dir, os.listdir(source_dir), shallow=False)
    ...
    for skill in os.listdir(source_dir):
        ...
        if source_skill.exists() and mirror_skill.exists():
            assert filecmp.cmp(source_skill, mirror_skill, shallow=False), f"Skill mirror mismatch: {skill}"
