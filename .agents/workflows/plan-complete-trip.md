...
3. Run independent destination, relevant ground transport, broad-activity, and applicable safety obligations. Use `flight-search` and its full matrix when fares are requested or flight choice controls dates/route. Otherwise record provisional airports and arrival/departure windows without inventing a flight or price.
4. `accommodation-researcher` depends on viable candidate dates, which may come from the user's brief without `flight-search`. Run lodging, local food/photo discovery, and activity refinement after date/area gates. Reuse source IDs.
...
8. Expand accepted artifacts once. Show a feasible experience-led plan with regional food and photo opportunities when relevant. Mark unpriced flights and exact-date lodging gaps; keep booking readiness false until critical quotes and logistics pass.
