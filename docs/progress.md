...
| **Phase 9** | Remote MCP & Distribution Pack | **Terminée (Consolidée)** | Découplage en 2 produits réutilisables : Pack de skills autonome (`packages/travel-skills`, installateur multiplateforme, manifest, zéro dépendance tierce) et Serveur MCP distant Streamable HTTP (`src/ultimate_travel_agent/mcp/http_server.py`, `/health`, `/ready`, `/version`, `/mcp`). Sécurité avancée : authentification par token en temps constant, limitation de débit par bucket de jetons, plafond de charge utile 1 Mo, traçage X-Request-ID, assainissement des logs, CORS configurable. Registre de providers dynamique via variables serveur (`TRAVEL_PROVIDER_*`) et YAML. Fichiers de déploiement multi-cibles (Dockerfile utilisateur non-root, docker-compose, Railway, Render, Google Cloud Run, Fly.io). Exemples de configuration pour Antigravity, Claude Code et Cursor. 124 tests automatisés passants à 100% hors-ligne. |
