"""GetYourGuide Partner API provider (read-only activities catalog)."""
...
class GetYourGuideActivityProvider(Provider):
    """GetYourGuide Partner API provider.

    Strictly read-only catalog exploration; no in-app booking or payment execution.
    """
    ...
    def __init__(
        ...
        super().__init__(
            ...
            supported_countries=["*"],
            capabilities=["activity_search", "guided_tours", "category_filter"],
        )
        self.api_key = api_key or os.getenv("GETYOURGUIDE_API_KEY", "")
    ...
    def normalize_result(self, raw: Dict[str, Any]) -> ProviderResultItem:
        return ProviderResultItem(
            ...
            source_url="https://www.getyourguide.com",
            ...
            title=raw.get("title", "Experience"),
            description=f"GetYourGuide tour: {raw.get('category')}",
            price=raw.get("cost_per_person"),
            rating=raw.get("rating", 4.6),
            details=raw,
        )
    ...
    def search(self, **kwargs: Any) -> ProviderSearchResult:
        if self.mode == ProviderMode.LIVE:
            if not self.is_configured():
                raise ProviderConfigurationError("GetYourGuide API key not configured. Set GETYOURGUIDE_API_KEY in .env.")
            raise ProviderConfigurationError("Live API calls to GetYourGuide disabled during development/testing.")
