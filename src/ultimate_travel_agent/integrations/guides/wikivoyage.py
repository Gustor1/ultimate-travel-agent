        ...
        if self.mode == ProviderMode.LIVE:
            if not self.is_configured():
                raise ProviderConfigurationError(
                    "Wikivoyage live guide feed is not activated. Set ENABLE_LIVE_KEYLESS_APIS=true in .env or run in offline/mock mode."
                )
            raise ProviderConfigurationError("Live API calls to Wikivoyage disabled during development/testing.")
