        ...
        if self.mode == ProviderMode.LIVE:
            if not self.is_configured():
                raise ProviderConfigurationError(
                    "ECB live currency feed is not activated. Set ENABLE_LIVE_KEYLESS_APIS=true in .env or run in offline/mock mode."
                )
            raise ProviderConfigurationError("Live API calls to ECB disabled during development/testing.")
